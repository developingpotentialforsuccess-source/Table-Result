...
const TEMPLATE_2A = SYSTEM_TEMPLATES[6].levels[0] as Level;
const TEMPLATE_2B = SYSTEM_TEMPLATES[7].levels[0] as Level;
const TEMPLATE_3A = SYSTEM_TEMPLATES[4].levels[0] as Level;
const TEMPLATE_3B = SYSTEM_TEMPLATES[5].levels[0] as Level;
const TEMPLATE_4A = SYSTEM_TEMPLATES[2].levels[0] as Level;
const TEMPLATE_4B = SYSTEM_TEMPLATES[3].levels[0] as Level;
const TEMPLATE_100_PERCENT = SYSTEM_TEMPLATES[8].levels[0].subjects;
const TEMPLATE_LEVEL_1A = SYSTEM_TEMPLATES[9].levels[0].subjects;
const TEMPLATE_LEVEL_1B = SYSTEM_TEMPLATES[10].levels[0].subjects;
const TEMPLATE_PRE2A_II = SYSTEM_TEMPLATES[12].levels[0] as Level;
const TEMPLATE_FOUNDATION_A = SYSTEM_TEMPLATES[13].levels[0] as Level;

export default function SettingsModal({ level, levels, onUpdateLevel, onReplaceLevels, onClose, paperStyle, onUpdatePaperStyle, wallpaper, onUpdateWallpaper, gridLineLevel, onUpdateGridLineLevel, settings, onUpdateSettings, onOpenTemplateModal, classRecords }: Props) {
  const [activeTab, setActiveTab] = useState<'level' | 'grading' | 'templates' | 'appearance' | 'account' | 'guide' | 'monitoring' | 'backup'>('level');
  ...
  const getScaleValues = (mode: 'full' | 'midterm' | 'final', type: 'grade' | 'status') => {
    if (type === 'status') {
      ...
      if (mode === 'full') return settings.gradingScale || [{ grade: 'A', minScore: 90 }, { grade: 'B', minScore: 80 }, { grade: 'C', minScore: 70 }, { grade: 'D', minScore: 60 }, { grade: 'F', minScore: 0 }];
      if (mode === 'midterm') return settings.midtermGradingScale || [{ grade: 'A', minScore: 90 }, { grade: 'B', minScore: 80 }, { grade: 'C', minScore: 70 }, { grade: 'D', minScore: 60 }, { grade: 'F', minScore: 0 }];
      return settings.finalGradingScale || [{ grade: 'A', minScore: 90 }, { grade: 'B', minScore: 80 }, { grade: 'C', minScore: 70 }, { grade: 'D', minScore: 60 }, { grade: 'F', minScore: 0 }];
    }
  };
  ...
  const applyTemplateToCurrent = () => {
    handleApplyToCurrentLevel([{ id: 'standard', name: 'Standard 100% Weighted Level', subjects: TEMPLATE_100_PERCENT, gradingScale: [] }]);
  };
              ...
              {editingTemplate ? (
                  ...
                  <div className="mt-auto pt-4 border-t border-slate-100 flex flex-col gap-2">
                    <div className="flex flex-col sm:flex-row gap-2">
                      <button 
                        ...
                        className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors cursor-pointer"
                        title="Apply to Current Class"
                      >
                        <Copy className="w-4 h-4" /> Apply to Class
                      </button>
                      <button 
                        onClick={() => handleDuplicateTemplate({
                          ...
                          authorName: 'System',
                          levels: [{ id: 'standard', name: 'Standard Level', subjects: TEMPLATE_100_PERCENT, gradingScale: [] }]
                        })}
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col">
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div>
                      <h4 className="font-semibold text-slate-800 text-lg">Level 1A Template</h4>
                      <p className="text-sm text-slate-500 mt-1">ALPHABETS (50%), CONVERSATION (50%). Active for Mid-Term, Final Test, and Final Term Result without homework/assignment scores.</p>
                    </div>
                  </div>
                  <div className="mt-auto pt-4 border-t border-slate-100 flex flex-col gap-2">
                    <div className="flex flex-col sm:flex-row gap-2">
                      <button 
                        onClick={() => handleApplyToCurrentLevel([{ id: 'l1_template', name: 'Level 1A Template', subjects: TEMPLATE_LEVEL_1A, gradingScale: [] }])}
                        className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-white bg-orange-600 rounded-lg hover:bg-orange-700 transition-colors cursor-pointer"
                        title="Apply to Current Class"
                      >
                        <Copy className="w-4 h-4" /> Apply to Class
                      </button>
                      <button 
                        onClick={() => handleDuplicateTemplate({
                          ...
                          authorName: 'System',
                          levels: [{ id: 'l1_template_level', name: 'Level 1A', subjects: TEMPLATE_LEVEL_1A, gradingScale: [] }]
                        })}
                        className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-slate-700 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors cursor-pointer"
                        title="Duplicate to Library"
                      >
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col">
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div>
                      <h4 className="font-semibold text-slate-800 text-lg">Level 1B Template (Official)</h4>
                      <p className="text-sm text-slate-500 mt-1">Official Level 1B with Phonics, Vocabulary, Alphabet Sounds, and Termly Exam Metrics (Midterm/Final Test weights).</p>
                    </div>
                  </div>
                  <div className="mt-auto pt-4 border-t border-slate-100 flex flex-col gap-2">
                    <div className="flex flex-col sm:flex-row gap-2">
                      <button 
                        onClick={() => handleApplyToCurrentLevel([{ id: 'l1b_template', name: 'Level 1B Template', subjects: TEMPLATE_LEVEL_1B, gradingScale: [] }])}
                        className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition-colors cursor-pointer"
                        title="Apply to Current Class"
                      >
                        <Copy className="w-4 h-4" /> Apply to Class
                      </button>
                      <button 
                        onClick={() => handleDuplicateTemplate({
                          ...
                          authorName: 'System',
                          levels: [{ id: 'l1b_template_level', name: 'Level 1B', subjects: TEMPLATE_LEVEL_1B, gradingScale: [] }]
                        })}
                        className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-slate-700 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors cursor-pointer"
                        title="Duplicate to Library"
                      >
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col">
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500 mt-1">Includes Phonics, Reading & Writing, Speaking & Vocabulary, Oral & Written, and Termly Metrics with standard grade and status scales.</p>
                    </div>
                  </div>
                  <div className="mt-auto pt-4 border-t border-slate-100 flex flex-col gap-2">
                    <div className="flex flex-col sm:flex-row gap-2">
                      <button 
                        onClick={() => handleApplyToCurrentLevel([TEMPLATE_PRE2A_II])}
                        className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-white bg-purple-600 rounded-lg hover:bg-purple-700 transition-colors cursor-pointer"
                        title="Apply to Current Class"
                      >
                        <Copy className="w-4 h-4" /> Apply to Class
                      </button>
                      <button 
                        onClick={() => handleDuplicateTemplate({
                          id: 'pre2a_ii_template_lib',
                          name: 'Level Pre-2A II Template (Official)',
                          authorName: 'System',
                          levels: [TEMPLATE_PRE2A_II]
                        })}
                        className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-slate-700 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors cursor-pointer"
                        title="Duplicate to Library"
                      >
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col">
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div>
                      <h4 className="font-semibold text-slate-800 text-lg">Level Foundation A Template (Official)</h4>
                      <p className="text-sm text-slate-500 mt-1">Includes Vocabulary & Dictation, Conversation, Speaking, Traffic Law, and detailed Termly Metrics with official weighting.</p>
                    </div>
                  </div>
                  <div className="mt-auto pt-4 border-t border-slate-100 flex flex-col gap-2">
                    <div className="flex flex-col sm:flex-row gap-2">
                      <button 
                        onClick={() => handleApplyToCurrentLevel([TEMPLATE_FOUNDATION_A])}
                        className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-white bg-amber-600 rounded-lg hover:bg-amber-700 transition-colors cursor-pointer"
                        title="Apply to Current Class"
                      >
                        <Copy className="w-4 h-4" /> Apply to Class
                      ...
                      <button 
                        onClick={() => handleDuplicateTemplate({
                          id: 'level_foundation_a_template_lib',
                          name: 'Level Foundation A Template (Official)',
                          authorName: 'System',
                          levels: [TEMPLATE_FOUNDATION_A]
                        })}
                        className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-slate-700 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors cursor-pointer"
                        title="Duplicate to Library"
                      >
