    ...
    if (isHidden) {
      ...
      const visibleCategoriesForSubject = subject.categories.filter((category) => {
        ...
        if (resultMode === 'full') {
          ...
          // In midterm mode:
          // 1. Hide anything explicitly marked as "Final"
          if (isFin) return false;
          // 2. Hide "Midterm" categories unless they have explicit weight > 0
          if (isMid) {
          ...
          // In final mode:
          // 1. Hide anything explicitly marked as "Midterm"
          if (isMid) return false;
          // 2. Hide "Final" categories unless they have explicit weight > 0
          if (isFin) {
      ...
      // Add Exam Columns (Only in midterm/final modes if weight > 0, or if no matching category exists in full mode)
      ...
      if ((resultMode === 'midterm' || (resultMode === 'full' && !hasMidtermCat))) {
      ...
      if ((resultMode === 'final' || (resultMode === 'full' && !hasFinalCat))) {
        ...
        const calculateModePct = (mode: 'midterm' | 'final' | 'full') => {
          ...
          subject.categories.forEach(cat => {
            ...
            const activeWeight = mode === 'midterm'
              ? (cat.midtermWeight !== undefined ? cat.midtermWeight : (isMidtermCategory(cat.name) ? cat.weight : 0))
              : mode === 'final'
                ? (cat.finalWeight !== undefined ? cat.finalWeight : (isFinalCategory(cat.name) ? cat.weight : 0))
                : cat.weight;
    ...
    <div className="flex flex-col gap-4">
      <div className="overflow-x-auto custom-scrollbar min-h-[420px] md:min-h-[520px]">
        ...
        >
          ...
          {/* Row 2: Categories */}
          {(resultMode === 'full' || resultMode === 'midterm' || resultMode === 'final') && (
            <tr>
            ...
            <tr>
              {itemCols.map((ic, i) => {
                  ...
                  return (
                    ...
                    >
                      {isHidden ? (
                        <span className="text-[10px] italic">Hidden</span>
                      ) : ic.isAvg ? (
            ...
            return (
              <React.Fragment key={student.id}>
                      ...
                      >
                        <div className="flex flex-col items-center gap-1">
                          <input
                            ...
                            placeholder={weighted.toFixed(1)}
                          />
                          <div className={`text-[8px] uppercase font-bold tracking-tighter ${isManual ? 'text-blue-500' : 'text-slate-400'}`}>
                ...
                <td className={`px-2 py-2 ${scoreBg} text-center border-l border-b ${gridStyles.totalBorderClass} min-w-[70px]`}>
                  <div className="flex flex-col items-center">
                    <span className="text-[8px] uppercase text-slate-400 mb-0.5 font-bold">Grade</span>
                    <span
                ...
                <td className={`px-2 py-2 ${scoreBg} text-center border-l border-b ${gridStyles.totalBorderClass} min-w-[70px]`}>
                  <div className="flex flex-col items-center">
                    <span className="text-[8px] uppercase text-slate-400 mb-0.5 font-bold">Status</span>
                    <span
