...
import { saveLevel, saveClassRecord, saveStudent } from './firestoreUtils';
...
export async function exportFullBackup(userId: string, levels: Level[], classRecords: ClassRecord[]): Promise<void> {
  try {
    ...
    for (const cr of classRecords) {
      ...
      const studentsPath = `users/${userId}/classes/${cr.id}/students`;
      const studentsSnapshot = await getDocs(collection(db, 'users', userId, 'classes', cr.id, 'students'));
      const studentsList = studentsSnapshot.docs.map(d => d.data() as Student);
