    ...
    currentLevel.subjects.forEach(subject => {
      const calculateModePct = (mode: 'midterm' | 'final' | 'full') => {
        ...
        subject.categories.forEach(cat => {
          const isMid = isMidtermCategory(cat.name);
          const isFin = isFinalCategory(cat.name);
          
          if (mode === 'midterm' && isFin && cat.midtermWeight === undefined) return;
          if (mode === 'final' && isMid && cat.finalWeight === undefined) return;
      ...
      if (resultMode === 'midterm') {
        ...
        hasSubjectScore = hasMidScore || subject.categories.some(cat => {
          const isMid = isMidtermCategory(cat.name);
          if (!isMid && cat.midtermWeight === undefined) return false;
          if (isFinalCategory(cat.name) && cat.midtermWeight === undefined) return false;
          for (let i = 0; i < cat.itemCount; i++) {
        ...
        hasSubjectScore = hasFinalScore || subject.categories.some(cat => {
          const isFin = isFinalCategory(cat.name);
          if (!isFin && cat.finalWeight === undefined) return false;
          if (isMidtermCategory(cat.name) && cat.finalWeight === undefined) return false;
          for (let i = 0; i < cat.itemCount; i++) {
            if (typeof student.scores[`${cat.id}_${i}`] === 'number') return true;
          }
          return false;
    ...
    currentLevel.subjects.forEach(subject => {
      ...
      if (detailMode === 'categories' || detailMode === 'both') {
        subject.categories.forEach(cat => {
          ...
          if (resultMode === 'midterm' && !isMid && cat.midtermWeight === undefined) return;
          if (resultMode === 'midterm' && isFin && cat.midtermWeight === undefined) return;
          
          if (resultMode === 'final' && !isFin && cat.finalWeight === undefined) return;
          if (resultMode === 'final' && isMid && cat.finalWeight === undefined) return;
          ...
          const weight = resultMode === 'midterm' 
            ? (cat.midtermWeight !== undefined ? cat.midtermWeight : (isMid ? cat.weight : 0))
            : resultMode === 'final'
              ? (cat.finalWeight !== undefined ? cat.finalWeight : (isFin ? cat.weight : 0))
              : cat.weight;
